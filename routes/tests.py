...
class AllTestsCase(TestCase):
    ...
    def test_model_train_duplicate(self):
        ...
        train = Train(name='t165' ,from_city=self.city_A, to_city=self.city_B, travel_time=129)
        with self.assertRaises(ValidationError):
            train.full_clean()

    def test_model_train_train2_duplicate(self):
        '''Тестирование при возникновении ошибок при создании дупликата поезда'''
        train = Train(name='t15' ,from_city=self.city_A, to_city=self.city_B, travel_time=9)
        with self.assertRaises(ValidationError):
            train.full_clean()
        try:
            train.full_clean()
    ...
    def test_home_routes_view(self):
        ...
        self.assertTemplateUsed(response, template_name='routes/find_rout.html')
        self.assertEqual(response.resolver_match.func, routes_view.find_rout)
